...
//流程，只有外部触发以后才会启动芯片。其实就是需要一个按钮触发它的启动流程。  
  ...
  SampleApp_Group.ID = SAMPLEAPP_FLASH_GROUP; //0x0001;//组号
...
void SampleApp_MessageMSGCB( afIncomingMSGPacket_t *pkt )
{
  ...
  switch ( pkt->clusterId ) //判断簇ID
  {
   ...
   case SAMPLEAPP_P2P_CLUSTERID:
      HalUARTWrite(1, "Rx:", 3);       //提示接收到数据
      HalUARTWrite(1, pkt->cmd.Data, pkt->cmd.DataLength); //串口输出接收到的数据
      HalUARTWrite(1, "\n", 1);         // 回车换行
      break; 
  }
}
