...
/*
* Write/read the global configuration values into/from flash
*/
ZStatus_t wsGloalConfigCB( uint16 clusterId, uint16 attrId, uint8 oper, uint8 *pValue, uint16 *pLen ){
  //Take the config as a string to send, so the first byte is the string length
  if(clusterId==ZCL_CLUSTER_ID_GEN_BASIC && attrId == ATTRID_WATER_SWITCH_NV_CONFIG){
    if(oper==ZCL_OPER_WRITE){
      //write
      osal_memcpy( &zclWATERSWITCH_NvConfig, pValue+1, sizeof(zclWATERSWITCH_NvConfig) );
      //Save to flash
      if(osal_nv_write( ZCD_NV_WATER_SWITCH_CONFIG, 0, sizeof(zclWATERSWITCH_NvConfig), &zclWATERSWITCH_NvConfig )!=SUCCESS){
        LOG_OUTPUT(LOG_ERROR,  "Nv config save error\r\n");
        return ZCL_STATUS_SOFTWARE_FAILURE;
      }
    } else if(oper==ZCL_OPER_READ){
      //read
      *pLen = sizeof(zclWATERSWITCH_NvConfig)+1;
      *pValue = *pLen;
      osal_memcpy( pValue+1, &zclWATERSWITCH_NvConfig, sizeof(zclWATERSWITCH_NvConfig) );
      return ZCL_STATUS_SUCCESS;
    } else if(oper==ZCL_OPER_LEN){
      //Get read length
      *pLen = sizeof(zclWATERSWITCH_NvConfig)+1;
      return ZCL_STATUS_SUCCESS;
    } else {
      LOG_OUTPUT(LOG_ERROR,  "Wrong attribute request\r\n");
      return ZCL_STATUS_SOFTWARE_FAILURE;
    }
  }
  return ZCL_STATUS_SUCCESS;
                   ...
                   zclWATERSWITCH_Occupancy = *pdata;
                           ...
                           reportRec->attrID == ATTRID_MS_FLOW_MEASUREMENT_MEASURED_VALUE){
                             ...
                             if(pInMsg->srcAddr.addr.shortAddr == WaterSwitch_TempDstAddr.addr.shortAddr){
                               ...
                               waterEntering = *pdata;
                               if(waterEntering){
                                 device_Status |= SALOR_WATER_ENTERING;
                               } else {
                                 device_Status &= ~SALOR_WATER_ENTERING;
                               }
...
void GetEnvTemp(){
  envTemp = (ReadAdcValue()>>4)/4.5-587.5 + zclWATERSWITCH_NvConfig.tempCalibration;
...
uint8 DecideWorkMode(){
  ...
  if(zclWATERSWITCH_OnOffSwitch == AUTO_CONTROL || zclWATERSWITCH_OnOff == PENDING) {
    ...
    if((device_Status & PUMP_ONLINE) && (device_Status & TEMP_ONLINE)){
      if(zclWATERSWITCH_OnOff == SALOR_OFF){
        //Now salor is off
        
        if(((GET_WATER_TEMP>=zclWATERSWITCH_NvConfig.winterSwtichTemp+1) //winter
            ||(envTemp>=zclWATERSWITCH_NvConfig.winterThreshold && GET_WATER_TEMP>=zclWATERSWITCH_NvConfig.summerSwitchTemp+1))  //summer
        ...
        if(zclWATERSWITCH_Occupancy==0 || //Too less water
           (envTemp< zclWATERSWITCH_NvConfig.winterThreshold && GET_WATER_TEMP<=zclWATERSWITCH_NvConfig.winterSwtichTemp-1 && (!salorWaterUsing))||     //winter
             (GET_WATER_TEMP<=zclWATERSWITCH_NvConfig.summerSwitchTemp-1 && (!salorWaterUsing)))   //summer
