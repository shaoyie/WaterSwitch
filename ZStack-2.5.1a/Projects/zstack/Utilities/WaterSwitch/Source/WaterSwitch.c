...
uint16 WaterSwitch_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & WATERSWITCH_SEND_MSG_EVT )
  {
    // Send "the" message
    WaterSwitch_SendTheMessage();
    ...
    // return unprocessed events
    return (events ^ WATERSWITCH_SEND_MSG_EVT);
  }
